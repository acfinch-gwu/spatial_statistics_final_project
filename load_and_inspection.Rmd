...
# Define the bbox values
xmin <- -83.6751
ymin <-  36.5407
xmax <- -75.0489
ymax <-  39.7230

# Create an 'st_bbox' and then convert to an sf POLYGON
dmv_bbox <- st_bbox(c(xmin = xmin, ymin = ymin, xmax = xmax, ymax = ymax),
                    crs = st_crs(4326))

# as sfc (simple feature geometry list)
dmv_sfc  <- st_as_sfc(dmv_bbox)

# wrap in an sf object
dmv_sf   <- st_sf(geometry = dmv_sfc) |> st_buffer(0.01)


nfhl_get(AOI = dmv_sf, layer = 28)
```
